...
function comparaPrendaPedido_add($idSubCateProdADD_ ){
    global $db;
    global $otNOW;
    $id_prenda_seleccionada = "";
    $dataQuery = array();
    $db->where ("id_solici_promo", $otNOW);
    $db->where ("id_subcatego_producto", $idSubCateProdADD_);
    $query = $db->getOne('especifica_prod_pedido');
    $id_prenda_pedido = $query['id_subcatego_producto'];
    
    ////////////////////////
    
    $db->where ("id_subcatego_producto", $id_prenda_pedido);  
    $db->where ("kit", "adicional");    
    $queryPD = $db->getOne('pack_dotacion_user');
    $id_prenda_seleccionada = $queryPD['id_subcatego_producto'];
    
    ////////////////////////
    
    return $id_prenda_seleccionada;
}


function comparaPrendaPedidoDot($idSubCateProd_, $kitvalue_ ){
    global $db;
    global $otNOW;
    $db->where ("id_subcatego_producto", $idSubCateProd_);  
    $db->where ("kit", $kitvalue_);    
    $queryPD = $db->getOne('pack_dotacion_user');
    $id_prenda_seleccionada = $queryPD['id_subcatego_producto'];
    return $id_prenda_seleccionada;
}

function comparaPrendaPedido($idSubCateProd, $kitvalue ){
    global $db;
    global $otNOW;
    $id_prenda_seleccionada = "";
    $dataQuery = array();
    ...
    ////////////////////////
    $id_prenda_seleccionada = comparaPrendaPedidoDot($id_prenda_pedido, $kitvalue );
    
    /*$db->where ("id_subcatego_producto", $id_prenda_pedido);  
    $db->where ("kit", $kitvalue);    
    $queryPD = $db->getOne('pack_dotacion_user');
    $id_prenda_seleccionada = $queryPD['id_subcatego_producto'];*/
    
    /*$rawSQL = "SELECT dot.id_subcatego_producto, dot.kit";
    $rawSQL .= " FROM especifica_prod_pedido ord, pack_dotacion_user dot ";
    $rawSQL .= "WHERE ord.id_solici_promo = '$otNOW' AND ord.id_subcatego_producto = '$idSubCateProd' AND dot.id_subcatego_producto = '$idSubCateProd'  AND dot.kit = '$kitvalue' AND dot.id_subcatego_producto = ord.id_subcatego_producto LIMIT 0,1";
    
    $queryPD = $db->rawQuery ($rawSQL);
    
    $ResuqueryPD = count($queryPD);
    if ($ResuqueryPD > 0){
        foreach ($queryPD as $queryPDkey) { 
            $id_prenda_seleccionada = $queryPD;    
        }
        return $id_prenda_seleccionada;
    }*/
...
$db->where('id_account_user',$idSSUser);//$idSSUser
$db->where('kit','adicional');
...
$grupoCategoriasUsuario = array();
$grupoCategorias = array();
//echo "<pre>";
...
if(is_array($categorias_seccion) && count($categorias_seccion)>0){
    ...
    //if(is_array($grupoCategorias) && !empty($grupoCategorias) ){
...
if(isset($_GET['tagcatadd'])){
    $kit_explorar_add = $_GET['tagcatadd'];
    $kit_explorar_add = (string)$kit_explorar_add; 
    $kit_explorar_add = $db->escape($kit_explorar_add);    
    $input_kit_adicional = "<input type='hidden' name='tagcataddinp' value='".$kit_explorar_add."' />";
}elseif(isset($_GET['tagcataddinp'])){
    $kit_explorar_add = $_GET['tagcataddinp'];
    $kit_explorar_add = (string)$kit_explorar_add; 
    $kit_explorar_add = $db->escape($kit_explorar_add);    
    $input_kit_adicional = "<input type='hidden' name='tagcataddinp' value='".$kit_explorar_add."' />";
}
...
if($kit_explorar_add !=""){
	...
	$acti_adicional_migaja = "";
	$acti_prendaexp_migaja = "hidden";
	$acti_kit_migaja = "hidden";
	$acti_home_migaja = "";
}elseif($prenda_activa!= ""){
    $acti_migaja = 3;
    $acti_prenda_migaja = "active";
...
$path_img_adicional = "";
$id_prenda_pedido_add = "";

$path_url_nav_add = "";
$print_img_adicional = "";
$nombre_adicional_catalogo = "";
...
//}
//sksort($prenda_adicional_usuario, 'tags_depatament_produsts', true);
//if(count($grupoCategoriasUsuario) > 0){
    ...
    if(is_array($paquete_adicional_usuario) && !empty($paquete_adicional_usuario) /*&& $varcontrol == "suputamadre"*/){
        foreach($paquete_adicional_usuario as $paulKey){
            ...
            if(is_array($categorias_adicional_usuario)){
                foreach($categorias_adicional_usuario as $cauVal){
                    ...
                    $path_url_nav_add = $pathmm.$takeOrderDir."/inicio/?steps=cuatro&tagcatadd=adicional";
            ...
            if(is_array($prenda_adicional_usuario)){
                ...
                foreach($prenda_adicional_usuario as $pauVal){
                    ...
                    $tagColectionPrenda_add = $pauVal['tags_depatament_produsts'];
                    $typeKitCateProd_add = "";//$pauVal['tipo_kit_4user'];
                    ...
                    $id_prenda_pedido_add = comparaPrendaPedido_add($idSubCateProd_add);

            
                    //IMPRIME LAS PRENDAS CORRESPONDIENTES AL KIT ACTIVADO POR EL USUARIO
                    //if($prendas_explorar == $idCateProd_add){
                    //if(isset($_GET['tagcatadd']) && isset($_GET['tagcatadd']) == "adicional"){
                    if($kit_explorar_add == "adicional"){

                        if($id_prenda_pedido_add == $idSubCateProd_add){
                            ...
                            $linkAdicional = "<a href='#' class='pzskit' name='".$nomeSubCateProd_add."' title='Pieza seleccionada' data-msj='Ya seleccionaste esta pieza, para cambiarla primero debes eliminarla desde la orden del pedido'>";
                            
                            $divDisabledPZ_add = "show";

                        }else{

                            if(isset($_GET['addl3']) && $_GET['addl3'] != ""){
                            $adicionalVar = (int)$_GET['addl3'];
                            $linkAdicional = "<a href='".$pathmm.$takeOrderDir."/browse/?catego=".$idSubCateProd_add."&l2=".$idCateProd_add."&tagcat=".$attr_kit_adicional."&addl3=".$idSubCateProd_add."&gender=".$tagColectionPrenda_add."&tagcatadd=adicional' >";
                            }else{
                                $linkAdicional = "<a href='".$pathmm.$takeOrderDir."/browse/?catego=".$idSubCateProd_add."&l2=".$idCateProd_add."&tagcat=".$attr_kit_adicional."&gender=".$tagColectionPrenda_add."&tagcatadd=adicional' >";    
                            }            
                            $divDisabledPZ_add = "hidden";
                        }
                    
                    	//if($prevColeccionPrendaAdd != $tagColectionPrenda_add){
                        $tmplLayoutItem .= "<div class='col-xs-12 col-sm-6 wrap-prod '>";// item subcate ".$wrapPZClass."
                        $tmplLayoutItem .= $linkAdicional;//"<a href='".$pathFile."/?catego=".$idSubCateProd."&l2=".$idCategoProd."'>";
                        $tmplLayoutItem .= "<div class='box box-widget widget-user-3 '>";// wrap subcate
                        $tmplLayoutItem .= "<div class='desabled ".$divDisabledPZ_add."'><i class='fa fa-check'></i><span>Seleccionado</span></div>";
                        ...
                        $tmplLayoutItem .= $nomeSubCateProd_add;
                        $tmplLayoutItem .= "</h3>";                            
                        ...
                        //$prevColeccionPrendaAdd = $tagColectionPrenda_add;
            ...
            if($kit_explorar_add == "adicional"){
                ...
                $layout_kit_usuario .= "</li>";    */
                $layout_kit_usuario .= $tmplLayoutItem;
            }
        ...
        if($kit_explorar_add == "adicional"){
            $path_migaja_nav_add = $pathmm.$takeOrderDir."/inicio/?steps=cuatro&tagcatadd=adicional";        
            $layout_migaja .= "<li role='presentation' class='".$acti_adicional_migaja."'><a href='".$path_migaja_nav_add."'>Unitario</a></li>";
...
if(count($grupoPrendas)>0){
    foreach($grupoPrendas as $gpuKit){
        ...
        $id_prenda_pedido = comparaPrendaPedido($idSubCateProd, $kit_explorar);
...
$valueGlobalProgess = 0;
$valueGlobalProgessFormat = 0;
if($total_articulos_dotacion > 0){
...
$valueProgresGlobal = 0;
$valueProgresGlobalFormat = 0;
if($maxPZAS>0){
