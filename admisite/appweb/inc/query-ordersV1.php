...
//SELECT `id_solici_promo`, `id_account_empre`, `cod_orden_compra`, `cod_promocional`, `nome_cliente`, `tel_cliente`, `mail_cliente`, `nome_empresa`, `representante_empresa`, `tel_empresa`, `cel_cliente`, `ciudad_empresa`, `dire_empresa`, `cc_nit_empresa`, `precio_venta_pedido`, `comision_venta`, `tipo_promocion`, `fecha_solicitud`, `hora_solicitud`, `ciudad_solicitud`, `id_account_user`, `estado_solicitud`, `metod_pago`, `datetime_publi`, `datetime_server` FROM `solicitud_pedido` WHERE 1
...
function queryEspeciPedido($idOrder_){
    ...
    $queryTbl = $db->get ("productos_filing pref", null, "scd.id_solici_promo, scd.cant_pedido, scd.nome_catego_product, scd.descri_catego_prod, scd.tags_depatament_produsts, scd.tipo_kit_4user, pref.id_prod_filing, pref.cod_venta_prod_filing, pref.cod_venta_descri_filing, pref.foto_producto_filing");
...
function ordersQuery($idOrder_){
    ...
    $queryTbl = $db->get ("solicitud_pedido", null, "estado_solicitud, id_solici_promo, id_account_empre, cod_orden_compra, nome_cliente, tel_cliente, mail_cliente, nome_empresa, representante_empresa, ciudad_empresa, id_account_user, fecha_solicitud, hora_solicitud");
    ...
    if ($rowQueryTbl > 0){
        foreach ($queryTbl as $qKey) {
            ...
            $cityDeliveryOrder = $qKey['ciudad_empresa'];
            ...
            $dataQuery[] = array(
                ...
                'ciudad_empresa' => $cityDeliveryOrder,
