...
function printFileErr($fileCheck_){
    ...
    //foreach ($fileCheck_ as $namesErr => $valCheck) {     
        foreach ($fileCheck_ as $keysErr => $valErr) { 
            ...
            $errTmpl.="<br>Erros:";
            if(!empty($typeFile)){ $errTmpl.="<br>Solo puedes usar archivos de imagen:&nbsp;&nbsp;" .$typeFile; }
            if(!empty($sizeFile)){ $errTmpl.="<br>La foto que intentas publicar, es muy pesada, el limite es:&nbsp;&nbsp;" .$sizeFile; }
            if(!empty($squareFile)){ $errTmpl.="<br>Las dimensiones (Ancho x Alto) maximas para as fotos son:&nbsp;&nbsp;" .$squareFile; }
            $errTmpl.="</li>";
        }
    //}
    ...
    switch($fotoProdType_){
        case "image/jpg":
            ...
            if(@imagejpeg($imagen_nueva, $filePathEnd, $qualityFile)){
                return true;
                $reloadDir = $filePathEnd;	
		$sec = "0";
		header("Refresh: $sec; url=$reloadDir");
		exit;
            }
		...
		$image->scaleImage($nuevo_ancho, $nuevo_alto, true);
		...
		$patchFilefinal = $filePathEnd;

		if($image->writeImage($patchFilefinal)){				                		                	                
			//unlink($uploadpath);
			clearstatcache(); 
			//$reloadDir = $filePathEnd;            	                
			$sec = "0";
			header("Refresh: $sec; url=$patchFilefinal");			
			return true;
		}
    ...
    foreach($errValida as $keyRules => $valRules){
        foreach($valRules as $key => $v){
            ...
            switch($errFiel){
                ...
                break; 
                case 'idpost':
                    $errValidaTmpl .="<li class='list-group-item list-group-item-danger'><b>NO ENCONTRADO</b>
                    <br>La pagina que intentas acceder no fue encontrada, o no existe. Si el error continua por favor contacta a soporte. Gracias</li>";
                break; 
            }
        }
    }
    ...
    if(unlink($rutaFileAlbum200) && unlink($rutaFileAlbum400) && unlink($rutaFileAlbum800) && rename_portada($rutaFileAlbum1200,$newRutaPortada1200)){
        ...
        if($db->delete('fotos_albun')){
            ...
            $errQueryTmpl .="<li class='list-group-item list-group-item-danger'><b>*** Algo salio mal ***</b><br>
                ...
                <br>Si el error continua, por favor entra en contacto con soporte. Gracias</li>";
            $errQueryTmpl .="</ul>";
            //$errQueryTmpl .="</section>";
        ...
        <br>En el momento de eliminar la imágen del servidor, sucedio un error. Por favor intente nuevamente mas tarde, o si prefieres puedes entrar en contacto con soporte.
        <br>Gracias</li>";                   
        $errValidaTmpl .="</ul>";
        //$errValidaTmpl .="</section>";
        return $errValidaTmpl;
            ...
            if(unlink($rutaLabel)){
                ...
                if($db->delete('albun_db')){                    
                    ...
                    $errQueryTmpl .="<li class='list-group-item list-group-item-danger'><b>*** Algo salio mal ***</b><br>
                        ...
                        <br>Erro: ".$erroQuery."
                        <br>Puedes intentar nuevamente elimminar el album
                        <br>Si el error continua, por favor entra en contacto con soporte. Gracias</li>";
                    $errQueryTmpl .="</ul>";
                    //$errQueryTmpl .="</section>";
                ...
                $errValidaTmpl ="<ul class='list-group text-left box75'>";
                $errValidaTmpl .="<li class='list-group-item list-group-item-danger'><b>Algo salio mal</b>
                <br>En el momento de eliminar la imágen del servidor, sucedio un error. Por favor intente nuevamente mas tarde, o si prefieres puedes entrar en contacto con soporte.<br>Gracias</li>";                   
                $errValidaTmpl .="</ul>";
                //$errValidaTmpl .="</section>";
                return $errValidaTmpl;            
            }
    ...
    if(rename_portada($rutaFileBig,$newRutaFile)){
        ...
        if($db->delete($tbldb_)){
            ...
            $errQueryTmpl .="<li class='list-group-item list-group-item-danger'><b>*** Algo salio mal ***</b><br>
                <br>Wrong: <b>".$tituSqlERR_."</b>
                <br>Erro: ".$erroQuery."
                <br>Puedes intentar nuevamente eliminar el archivo
                <br>Si el error continua por favor entra en contacto con soporte. Gracias</li>";
            $errQueryTmpl .="</ul>";
            //$errQueryTmpl .="</section>";
        ...
        $errValidaTmpl ="<ul class='list-group text-left box75'>";
        $errValidaTmpl .="<li class='list-group-item list-group-item-danger'><b>Algo salio mal</b>
        <br>En el momento de eliminar el archivo del servidor, ocurrio un problema. Intentalo de nuevo o entra en contacto con soporte. 
        <br> Gracias/li>";                   
        $errValidaTmpl .="</ul>";
        //$errValidaTmpl .="</section>";
        return $errValidaTmpl;
